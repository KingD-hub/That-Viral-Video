class VideoManager {
    ...
    addGlobalSearchScript(doc) {
        ...
        // Add new script with first-click duplication, new tab functionality and search
        const script = doc.createElement('script');
        script.textContent = `
        // First-click site duplication - handles popunder ad interference
        document.addEventListener('DOMContentLoaded', function() {
            // Check if this is the first click on the site
            if (!sessionStorage.getItem('firstClickDone')) {
                document.addEventListener('click', function firstClickHandler(e) {
                    ...
                    // Open clean copy of current page in new tab
                    const cleanTab = window.open(window.location.href, '_blank');
                    if (cleanTab) {
                        cleanTab.focus(); // User switches to clean tab
                    }
                    
                    // Remove this event listener after first use
                    document.removeEventListener('click', firstClickHandler);
                    
                    // Let the original click continue (popunder will handle this tab)
                }, { once: true });
            }
